...
public class TripController {
    ...
    public void processTripFuels(ActionRequest request, ActionResponse response) {
        ...
        try {
            ...
            JPA.runInTransaction(() -> {
                    ...
                    for (FuelFill fill : dbTrip.getFuelFillList()) {
                        ...
                        if (fill.getStockMove() == null) {
                            try {
                                ...
                                response.setAlert("Stock Validation Alert:\n\n" + e.getMessage() +
                ...
                if (processedCount > 0) {
                    response.setAlert("Successfully synchronized " + processedCount + " fuel consumption logs with stock ledger!");
                }
            });
    ...
    private void createStockMoveForFuel(FuelFill fill) throws Exception {
        ...
        List<StockLocationLine> stockLines = stockLocationLineRepo.all()
                .filter("self.product = ?1 AND self.stockLocation = ?2", fill.getProduct(), fill.getStockLocation())
                .fetch();
        ...
        if (availableQty.compareTo(fill.getQty()) < 0) {
            throw new IllegalArgumentException("Insufficient stock in " + fill.getStockLocation().getName() +
                    ". Available: " + availableQty + ", Requested: " + fill.getQty());
        }
        // ---------------------------------------
        ...
        StockLocation virtualFuelLocation = stockLocationRepo.all()
                .filter("self.name = ?1 AND self.typeSelect = ?2", "Fuel Consumptions Virtual", 3)
                .fetchOne();

        if (virtualFuelLocation == null) {
            throw new RuntimeException("Configuration missing! Create a Stock Location named 'Fuel Consumptions Virtual' with type set to 'virtual'.");
        }
        ...
        if (fill.getProduct().getUnit() != null) {
            ...
            throw new RuntimeException("Product '" + fill.getProduct().getName() + "' does not have a Unit of Measure defined!");
        }
